...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
  ...
  $uploadpath = $_SERVER["PHPRC"] .'/storage/'. $upload_dir . $img_name;       // full file path
  ...
  if($err == '') {
    if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) {
      ...
      $url = "https://images.affy.live/".$upload_dir . $img_name;
      ...
      $echo["error"]["message"] = "Unable to upload the file";
    }
  }else{
